...
func newFileConfig(create glossariescmd.CreateConfig) glossariescmd.FileConfig {
	var fc glossariescmd.FileConfig
	fc.Glossaries.Create = create
	return fc
}
...
func Test_resolveParams(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			setup:        defaultSetup,
			fileConfig:   newFileConfig(glossariescmd.CreateConfig{VerificationMode: true}),
			glossaryName: testGlossaryName,
			want: func() srv.CreateParams {
				p := baseWant()
				p.VerificationMode = true
		...
		},
		{
			...
			setup:        defaultSetup,
			fileConfig:   newFileConfig(glossariescmd.CreateConfig{LocaleIDs: []string{"fr-FR", "de-DE"}}),
			glossaryName: testGlossaryName,
			want: func() srv.CreateParams {
				p := baseWant()
		...
		},
		{
			...
			setup: defaultSetup,
			fileConfig: newFileConfig(glossariescmd.CreateConfig{
