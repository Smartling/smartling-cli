...
func Test_resolveParams(t *testing.T) {
	...
	params, err := resolveParams([]string{"h1", "h2"}, []string{"fr-FR"})
	if err != nil {
		t.Fatalf("resolveParams() error = %v", err)
	}
	if params.ProjectUID != "config-project-id" {
		t.Errorf("ProjectUID = %q, want config-project-id", params.ProjectUID)
	}
