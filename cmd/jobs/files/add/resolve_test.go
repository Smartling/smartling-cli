...
func Test_resolveParams(t *testing.T) {
	...
	params, err := resolveParams("aabbccdd1122", []string{"**/*.json"}, []string{"fr-FR"})
	if err != nil {
	...
	if len(params.FilePatterns) != 1 || params.FilePatterns[0] != "**/*.json" {
		t.Errorf("FilePatterns = %v, want [**/*.json]", params.FilePatterns)
	}
