...
func Test_resolveParams(t *testing.T) {
	...
	params, err := resolveParams("aabbccdd1122", []string{"old/*.xml"})
	if err != nil {
	...
	if len(params.FilePatterns) != 1 || params.FilePatterns[0] != "old/*.xml" {
		t.Errorf("FilePatterns = %v, want [old/*.xml]", params.FilePatterns)
	}
}
