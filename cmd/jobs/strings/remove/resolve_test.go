...
func Test_resolveParams(t *testing.T) {
	...
	params, err := resolveParams("aabbccdd1122", []string{"h1"}, []string{"fr-FR"})
	if err != nil {
	...
	if len(params.Hashcodes) != 1 || params.Hashcodes[0] != "h1" {
		t.Errorf("Hashcodes = %v, want [h1]", params.Hashcodes)
	}
	if len(params.LocaleIDs) != 1 || params.LocaleIDs[0] != "fr-FR" {
		t.Errorf("LocaleIDs = %v, want [fr-FR]", params.LocaleIDs)
	}
}
