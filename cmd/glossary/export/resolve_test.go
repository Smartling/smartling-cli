...
func newFileConfig(export glossarycmd.ExportConfig) glossarycmd.FileConfig {
	var fc glossarycmd.FileConfig
	fc.Glossary.Export = export
	return fc
}
...
func Test_resolveParams(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			fileConfig:        newFileConfig(glossarycmd.ExportConfig{FileType: "xlsx"}),
			glossaryUIDOrName: testGlossary,
			outFile:           testOutFile,
			want: func() srv.ExportParams {
				p := baseWant()
		...
		},
		{
			...
			fileConfig:        newFileConfig(glossarycmd.ExportConfig{TbxVersion: "v2"}),
			glossaryUIDOrName: testGlossary,
			outFile:           testOutFile,
			want: func() srv.ExportParams {
				p := baseWant()
		...
		},
		{
			...
			fileConfig:        newFileConfig(glossarycmd.ExportConfig{FileType: "csv", LocaleIDs: []string{"fr-FR"}}),
			glossaryUIDOrName: testGlossary,
			outFile:           testOutFile,
			want: func() srv.ExportParams {
				p := baseWant()
		...
		},
		{
			...
			setup:             func(t *testing.T) *cobra.Command { return makeCmd(t, noConfigPath, string(testAccount)) },
			fileConfig:        newFileConfig(glossarycmd.ExportConfig{FileType: "csv", SkipEntries: true}),
			glossaryUIDOrName: testGlossary,
			outFile:           testOutFile,
			want: func() srv.ExportParams {
				p := baseWant()
		...
		// ── Filter fields from fileConfig ─────────────────────────────────────
		{
			...
			setup: func(t *testing.T) *cobra.Command { return makeCmd(t, noConfigPath, string(testAccount)) },
			fileConfig: newFileConfig(glossarycmd.ExportConfig{
				FileType: "csv",
				Filter: glossarycmd.ExportFilterConfig{
					Query:      "checkout",
					EntryState: "ACTIVE",
					EntryUIDs:  []string{"uid-1", "uid-2"},
					LastModified: glossarycmd.LastModifiedConfig{
						Level: "GLOSSARY",
						Type:  "AFTER",
					},
				},
