...
// NewListCmd builds the `glossary list` command.
func NewListCmd(initializer glossarycmd.SrvInitializer) *cobra.Command {
	var name string
...
# List glossaries
  ...
  smartling-cli glossary list

# List glossaries with name "CLI"

  smartling-cli glossary list --name "CLI"

# List glossaries with name "CLI" and output in table

  smartling-cli glossary list --name "CLI" --output table
`,
		Args: cobra.NoArgs,
