...
func TestGlossaryExport(t *testing.T) {
	...
	subCommands := []string{"glossaries", "export"}
	...
	tests := []struct {
	...
	}{
		{
			...
			expectedOutputs:   []string{"Export a glossary's entries to a file", "file-type", "locale"},
		...
		},
		{
			...
			args:              []string{"glossaries", "--output", "table", "export", testGlossary, "--file-type", "csv"},
			expectedOutputs:   []string{"GLOSSARY UID", "OUTPUT FILE", "FILE TYPE", "BYTES WRITTEN"},
			unexpectedOutputs: []string{"DEBUG", "ERROR"},
			wantErr:           false,
		},
		{
			...
			args:              []string{"glossaries", "--output", "json", "export", testGlossary, "--file-type", "csv"},
