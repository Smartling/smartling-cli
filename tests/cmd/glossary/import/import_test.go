...
func exportCSV(t *testing.T, absDir, glossaryName, outName string) string {
	...
	cmd := exec.Command("./smartling-cli", "glossary", "export", glossaryName, outName, "--file-type", "csv")
	cmd.Dir = absDir
...
func TestGlossaryImport(t *testing.T) {
	...
	subCommands := []string{"glossary", "import"}
	...
	tests := []struct {
		...
		},
		{
			...
			args: func() []string {
				...
				return []string{"glossary", "--output", "table", "import", testGlossary, f}
			}(),
		...
		},
		{
			...
			args: func() []string {
				...
				return []string{"glossary", "--output", "json", "import", testGlossary, f}
			}(),
